...
from config.settings import HEATMAP_COLORSCALE, MIN_HEATMAP_HEIGHT, HEATMAP_ROW_HEIGHT
...
def render_activity_heatmap(matrix_data: pd.DataFrame, period_title: str):
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text="🎙️ Device (by Country)", font=dict(size=14))
        ),
        height=chart_height,
...
def render_country_bar_chart(status_df: pd.DataFrame):
    ...
    country_stats = status_df.groupby('Country').agg({
        'status': lambda x: [(x == 'Online').sum(), (x == 'Offline').sum()]
    }).reset_index()
