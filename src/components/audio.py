...
def render_audio_stats(stats: dict, total_stats: dict = None) -> None:
    ...
    if total_stats:
        render_info_section_header("🌐 Dataset Contribution", level="h4", style_class="dataset-contribution-header")
...
def render_audio_export_options(recordings: pd.DataFrame, site_name: str, audio_data: pd.DataFrame) -> None:
    ...
    with col1:
        if st.button("📥 Export Recording List", key="export_recordings"):
            csv_data = recordings[["Name", "recorded_at", "Path", "Size"]].to_csv(index=False)
            st.download_button(
