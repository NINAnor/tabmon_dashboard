...
def render_status_table(status_df: pd.DataFrame):
    ...
    display_cols = [
        "Cluster", "Site", "DeploymentID", "Country", 
        "status", "last_recorded", "days_since_last"
    ]
...
def render_summary_table(status_df: pd.DataFrame):
    ...
    if 'Country' in status_df.columns:
        summary = status_df.groupby('Country').agg({
            'status': lambda x: (x == 'Online').sum(),
            'DeploymentID': 'count'
        }).reset_index()
        
        summary.columns = ['Country', 'Online Devices', 'Total Devices']
        summary['Offline Devices'] = summary['Total Devices'] - summary['Online Devices']
        summary['Online Rate %'] = (summary['Online Devices'] / summary['Total Devices'] * 100).round(1)
